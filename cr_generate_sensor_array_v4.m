...
if ~isfield(S,'fullbody'), S.fullbody = 0; end
...
if S.fullbody
    ...
    % Create cylindrical grid around body
    % Y-axis is vertical (head-lumbar), so we vary X-Z plane
    z_range = max_z - min_z;
    x_range = max_x - min_x;
    
    % Create radial sampling: multiple circles at different Y heights
    radius = max(x_range, z_range) * 0.75;  % radius for ray origins
    n_angles = round(2 * pi * radius / S.resolution);  % angular samples
    
    [ygrid, theta_grid] = meshgrid(min_y:S.resolution:max_y, ...
                                    linspace(0, 2*pi, n_angles));
    
    % Convert to X,Z coordinates (circle around Y-axis)
    cx = mean([min_x, max_x]);  % center X
    cz = mean([min_z, max_z]);  % center Z
    
    xgrid = cx + radius * cos(theta_grid(:));
    zgrid = cz + radius * sin(theta_grid(:));
    ygrid = ygrid(:);
    
    % Ray directions point inward toward body center
    ray_dirs = [-cos(theta_grid(:)), zeros(numel(theta_grid),1), -sin(theta_grid(:))];
    
    fprintf('Raycasting subject surface (360°)...\n');
    plane_xyz = zeros(0,3);
    plane_n = zeros(0,3);
    ...
    for ii = 1:numel(xgrid)
        ...
        if ~isempty(P)
            hits = find(I);
            % Take first hit (closest to ray origin)
            if size(P,1) > 1
                dists = sqrt(sum((P - R.orig').^2, 2));
                [~, pid] = min(dists);
            else
                pid = 1;
            end
            
            nrm = nrms_sub(hits(pid),:);
            nrm = nrm ./ norm(nrm);
            
            % Check angle: only keep if ray is roughly aligned with inward normal
            ang = abs(acosd(dot(nrm, ray_dirs(ii,:))));
            ang = min(ang, 180 - ang);
...
normals_kept = normals_kept(keep,:);
...
% apply depth shift along surface normal
if S.fullbody
    % Use actual surface normals for depth offset
    grid_all = grid_kept + normals_kept * S.depth;
else
    % Use fixed ray direction
...
elseif S.triaxial && S.fullbody
    ...
    for i = 1:nSensors
        ...
        idx = (i-1)*3 + (1:3);
        grad.coilori(idx(1),:) = nrm';      % Radial (outward normal)
        grad.coilori(idx(2),:) = tang1;     % Tangent 1
        grad.coilori(idx(3),:) = tang2;     % Tangent 2
